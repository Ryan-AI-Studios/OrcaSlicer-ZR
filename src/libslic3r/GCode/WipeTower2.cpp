...
public:
	...
	WipeTowerWriter2& extrude_explicit(float x, float y, float e, float f = 0.f, bool record_length = false, bool limit_volumetric_flow = true)
	{
      ...
      float width = e * m_filpar[0].filament_area / (len * m_layer_height);
        ...
        // Always emit F on pure travels (even if it matches m_current_feedrate) so post-Tn G-code
        // re-states feedrate after the toolchange command. Default 500 mm/s (30000 mm/min).
        if (e == 0.f) {
            if (f == 0.f)
                f = (m_current_feedrate > 1.f) ? m_current_feedrate : 30000.f;
            m_gcode += set_format_F(f);
            m_current_feedrate = f;
        } else if (f != 0.f && f != m_current_feedrate) {
            if (limit_volumetric_flow && len + std::abs(e) > 0.f) {
