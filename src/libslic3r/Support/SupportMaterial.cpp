        ...
        Polygons            &contact_polygons)
    {
        ...
        Polygons bridges;
        {
            ...
            Polygons lower_grown_slices = expand(lower_layer_polygons,
                ...
                0.5f * float(scale_(spectrum_nozzle_mm_for_physical(
                    print_config.nozzle_diameter.values,
                    spectrum_physical_for_filament(
                        unsigned(std::max(0, layerm.region().config().outer_wall_filament_id.value)),
    ...
    const PrintObject &object, const std::vector<Polygons> &buildplate_covered, SupportGeneratorLayerStorage &layer_storage) const
{
    ...
    const coordf_t extrusion_width = m_object_config->line_width.get_abs_value(
        spectrum_nozzle_mm_for_physical(
            ...
            spectrum_physical_for_filament(
                unsigned(std::max(0, int(object.config().support_interface_filament))),
